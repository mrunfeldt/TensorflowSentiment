...
object PocMnist extends App{
  ...
  // Loading the graph
  val graphDef = readAllBytesOrExit(Paths.get(modelDir, "saved_model.pb"))

  // Infering
  val gModel = new Graph()
  gModel.importGraphDef(graphDef)
//  val session = new Session(gModel)
//  val result = session.runner().feed("input", inputTensor).fetch("output").run().get(0)
//  val rshape = result.shape()
//
//  println(s"Output number of dimension: $rshape")
}
